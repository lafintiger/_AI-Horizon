...
def generate_html_report():
    ...
    print("[ANALYSIS] Analyzing manually entered articles...")
    manual_results = analyze_manual_entries()
    
    # Merge both sets of results
    if manual_results:
        print(f"[SUCCESS] Found manual entries in {len(manual_results)} categories")
        results = merge_analysis_results(automated_results, manual_results)
    else:
        print("[INFO] No manual entries found")
        results = automated_results
    
    if not results:
        print("[ERROR] No analysis results found. Please run test_all_categories.py or add manual entries first.")
        return
...
PRIORITY SOURCES: CISO perspectives, security leadership studies, policy research, ethics papers"""
    ...
    print(f"[FILEPATH] {report_file}")
    print(f"[WEB] Open this file in your web browser to view the analysis")
